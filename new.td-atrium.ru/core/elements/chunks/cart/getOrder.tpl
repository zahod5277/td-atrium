<h3>Благодарим Вас за оформление заказа №{$order.num} на нашем сайте!</h3>
<h3>Подробности заказа:</h3>
<div class="col-xs-12">
    <h5>Получатель: {$address.receiver}</h5>
    <h5>E-mail: {$user.email}</h5>
    <h5>Контактный телефон: {$address.phone}</h5>
    <h5>Выбранный вариант оплаты: {$payment.name}</h5>
    <h5>Адрес доставки: {$address.city}, {$address.street}, {$address.building}</h5>
</div>
<h4>Подробности заказа продублированы на вашу электронную почту.</h4>
<h4>Если вы выбрали оплату картой, но по каким-то причинам прервали оплату, в письме вы найдете ссылку на продолжение оплаты</h4>
<div id="msCart">
